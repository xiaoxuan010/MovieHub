/* empty css                */
import { _ as _export_sfc, c as createElementBlock, b as createBaseVNode, a as createVNode, w as withCtx, r as resolveComponent, n as normalizeStyle, e as axios, f as api, E as ElMessage, i as ElMessageBox, o as openBlock, g as createTextVNode } from "./index-Zb4gIbIS.js";
const _sfc_main = {
  ...
  methods: {
    getTableData() {
      axios.get(api.apigetalldirector, {
        ...
        this.directorData = response.data.data;
        this.tableData = this.directorData.map((item) => ({
          id: item.id,
          name: item.name,
          description: item.description
        }));
        for (let i = 0; i < this.tableData.length; i++) {
          this.tableData[i].description = this.tableData[i].description.length > 100 ? this.tableData[i].description.slice(0, 100) + "..." : this.tableData[i].description;
        }
      });
    },
    handleEditSave() {
      ...
      axios.put(api.apiupdatedirector(this.editForm.id), updataform, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        }
      }).then((response) => {
    ...
    handleEdit(index, row) {
      this.editForm = { ...row };
      this.editForm.description = this.directorData[row.id - 1].description;
      this.editDialogVisible = true;
    },
    handleDelete(index, row) {
      ElMessageBox.confirm(`确定要删除此导演 ${row.name} 吗？`, "提示", {
    ...
    handleAddSave() {
      ...
      axios.post(api.apicreatedirector, addform, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        }
      }).then((response) => {
    ...
    createBaseVNode("div", _hoisted_2, [
      createVNode(_component_el_row, null, {
        default: withCtx(() => _cache[10] || (_cache[10] = [
          ...
          createBaseVNode("p", { class: "Adtitle2" }, "  导演管理", -1)
        ])),
        _: 1
      }),
      createVNode(_component_el_row, null, {
        default: withCtx(() => _cache[11] || (_cache[11] = [
          createBaseVNode("p", { class: "Adtitle3" }, "导演管理", -1)
        ])),
        _: 1
      })
    ]),
...
const Addirector = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render]]);
export {
  Addirector as default
};
